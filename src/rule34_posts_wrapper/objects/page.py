...
class Page(Sequence[Post]):
    ...
    def from_html(html: str, number: int, query: list[str]):
        ...
        for image in images:
            ...
            posts.append(Post(
                ...
                tags=tags,
                link=urllib3.util.parse_url("https://rule34.paheal.net/post/view/{identifier}"
    ...
    @validator(lambda query: not is_list_of_safe_strings(query),
               ValueError("Query must be a list of string consist of letter, numbers, dashes, underscores and "
                          "apostrophes"))
